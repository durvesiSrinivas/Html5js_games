...
let score = 0;
let gameInterval;
const gameSpeed = 280; // Milliseconds per game update
...
function update() { // actual game logic status here
  ...
  // Draw the food
  ctx.fillStyle = 'green'; //any element next to this line should be green filled
  ...
  snake.forEach((segment, index) => { // this is for each function contain function with three argu we use only two to iterate
    ctx.fillStyle = index === 0 ? 'yellow' : 'lime'; // Different color for the head and body in snake array
